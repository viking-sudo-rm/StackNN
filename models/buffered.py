...
# TODO: enqueue_all needs to be added to modularized Queue.
...
class BufferedController(AbstractController):
    ...
    def __init__(self, input_size, read_size, output_size, **args):
        ...
        self.linear = nn.Linear(input_size + self.get_read_size(),
                                self.N_ARGS + self.get_read_size() +
                                output_size)
        ...
        self.reg_tracker = InterfaceRegTracker(1.)
    ...
    def forward(self):
        ...
        read_params = F.sigmoid(
            output[:, :self.N_ARGS + self.get_read_size()])
        ...
        self.v = read_params[:, self.N_ARGS:].contiguous()

        self.read_stack(self.v, self.u, self.d)
        out = output[:, self.N_ARGS + self.get_read_size():].contiguous()
        self.buffer_out.forward(out, 0., self.e_out)

    def init_stack_and_buffer(self, batch_size, X, pad):
        super(BufferedController, self).init_stack(batch_size)
        ...
        self.buffer_in.set_reg_tracker(self.reg_tracker, Operation.pop)

        self.buffer_out = Queue(batch_size, self.input_size)
        self.e_out = Variable(torch.zeros(batch_size))
        self.buffer_out.set_reg_tracker(self.reg_tracker, Operation.push)
